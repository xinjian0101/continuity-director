...
function isCD(node) { return String(node?.comfyClass || node?.type || "").startsWith("CD"); }
...
function center() {
  ...
  if (finitePair(canvas?.graph_mouse)) return [Number(canvas.graph_mouse[0]), Number(canvas.graph_mouse[1])];
  const scaleValue = Number(canvas?.ds?.scale);
  const scale = Number.isFinite(scaleValue) && scaleValue > 0 ? scaleValue : 1;
  const offset = finitePair(canvas?.ds?.offset) ? [Number(canvas.ds.offset[0]), Number(canvas.ds.offset[1])] : [0, 0];
  return [(Number(canvas?.canvas?.width) || 1200) / 2 / scale - offset[0], (Number(canvas?.canvas?.height) || 800) / 2 / scale - offset[1]];
}
...
function style(node) {
  ...
  node.size = [Number.isFinite(width) ? Math.max(width, MIN_NODE_WIDTH) : MIN_NODE_WIDTH, Number.isFinite(height) ? Math.max(height, MIN_NODE_HEIGHT) : MIN_NODE_HEIGHT];
}

function add(type, pos, showToast = true) {
  try {
    const node = window.LiteGraph?.createNode?.(type);
    if (!node || !app.graph?.add) throw new Error(`Unable to create ${type}`);
    style(node);
    node.pos = finitePair(pos) ? [Number(pos[0]), Number(pos[1])] : center();
    app.graph.add(node);
    app.canvas?.setDirty?.(true, true);
    if (showToast) notify(t("added"), node.title || type);
    return node;
  } catch (error) {
    console.error("[Continuity Director] node creation failed", type, error);
    if (showToast) notify(t("addFailed"), type, "error");
    return null;
  }
}
...
function removeNodes(nodes) {
  for (const node of [...nodes].reverse()) {
    if (app.graph?.remove) app.graph.remove(node);
    else if (Array.isArray(app.graph?._nodes)) {
      const index = app.graph._nodes.indexOf(node);
      if (index >= 0) app.graph._nodes.splice(index, 1);
    }
  }
  app.canvas?.setDirty?.(true, true);
}
...
function dimensions(node) {
  const width = Number(node?.size?.[0]);
  const height = Number(node?.size?.[1]);
  return [Number.isFinite(width) ? Math.max(width, MIN_NODE_WIDTH) : MIN_NODE_WIDTH, Number.isFinite(height) ? Math.max(height, MIN_NODE_HEIGHT) : MIN_NODE_HEIGHT];
}

function layoutStarter(nodes, base) {
  const columns = [["CDProjectLock", "CDCharacterLock", "CDSceneLock"], ["CDShotLock"], ["CDManifestBuilder"], ["CDBatchDirector"], ["CDExecutionPlan", "CDExportPackage"]];
  const metrics = columns.map((types) => {
    ...
    return { nodes: columnNodes, sizes, width: Math.max(...sizes.map(([width]) => width), MIN_NODE_WIDTH), height: sizes.reduce((total, [, height]) => total + height, 0) + Math.max(0, sizes.length - 1) * STARTER_GAP_Y };
  });
  ...
  for (const column of metrics) {
    ...
    column.nodes.forEach((node, index) => { const [, height] = column.sizes[index]; node.pos = [Math.round(x), Math.round(y)]; y += height + STARTER_GAP_Y; });
...
function starter() {
  ...
  for (const type of types) {
    const node = add(type, base, false);
    if (!node) {
      removeNodes(created);
      notify(t("starterFailed"), type, "error");
      return false;
    }
    nodes.set(type, node);
    created.push(node);
  ...
  try {
    ...
    console.error("[Continuity Director] starter connection failed", error);
    removeNodes(created);
    notify(t("starterFailed"), String(error?.message || error), "error");
    return false;
  }
  app.canvas?.setDirty?.(true, true);
  notify(t("starterAdded"));
  return true;
}
...
function button(label, kind, handler, icon) { const item = el("button", `cd-button ${kind}`); item.append(ic(icon), document.createTextNode(label)); item.onclick = handler; return item; }

function header(host, language) {
  const wrapper = el("header", "cd-header"), brand = el("div", "cd-brand"), mark = el("div", "cd-mark"), copy = el("div", "cd-brand-copy");
  ...
  for (const [value, label] of [["en", "English"], ["zh-CN", "中文"], ["bilingual", "EN / 中文"]]) { const option = document.createElement("option"); option.value = value; option.textContent = label; option.selected = language === value; select.append(option); }
  select.onchange = () => { safeSet(KEY, select.value); render(host); };
  wrapper.append(brand, select);
  return wrapper;
}

function metric(icon, label, value, state = "") { const wrapper = el("div", `cd-metric ${state}`), visual = el("div", "cd-metric-icon"), copy = el("div", "cd-metric-copy"); visual.append(ic(icon)); copy.append(el("b", "", String(value)), el("span", "", label)); wrapper.append(visual, copy); return wrapper; }

function overview(language) {
  const state = stats(), view = el("section", "cd-view"), metrics = el("div", "cd-metrics");
  ...
  const quick = el("div", "cd-card"), quickCopy = el("div"), actions = el("div", "cd-actions");
  ...
  const health = el("div", "cd-card"), row = el("div", "cd-health");
...
function library(language) {
  const view = el("section", "cd-view"), head = el("div", "cd-section-head");
  head.append(el("h3", "", t("nodeLibrary", language)), el("span", "cd-chip", `v${VER}`));
  view.append(head);
  for (const [key, list] of Object.entries(groups)) {
    const group = el("div", "cd-group"), groupHead = el("div", "cd-group-head");
    groupHead.append(el("span", "", t(key, language)), el("span", "cd-chip", String(list.length)));
    group.append(groupHead);
    for (const [type, label, icon] of list) { const item = el("button", "cd-node-button"), visual = el("span", "cd-node-icon"), copy = el("span", "cd-node-copy"); visual.append(ic(icon)); copy.append(el("b", "", t(label, language)), el("small", "", type)); item.append(visual, copy, ic("pi-plus")); item.onclick = () => add(type); group.append(item); }
    view.append(group);
  }
  return view;
}
...
function guide(language) {
  const view = el("section", "cd-view"), timeline = el("div", "cd-timeline");
  view.append(el("h3", "", t("pipeline", language)));
  ["step1", "step2", "step3", "step4"].forEach((key, index) => { const row = el("div", "cd-step"); row.append(el("span", "cd-step-no", String(index + 1)), el("p", "", t(key, language))); timeline.append(row); });
...
function render(host) {
  if (!host) return;
  const language = lang(), root = el("div", "cd-dashboard"), tabs = el("nav", "cd-tabs"), main = el("main", "cd-content"), views = { overview: overview(language), nodes: library(language), guide: guide(language) };
  host.replaceChildren(); host.classList.add("cd-host"); root.append(header(host, language));
  const show = (id) => { host.dataset.cdTab = id; tabs.querySelectorAll("button").forEach((item) => item.classList.toggle("active", item.dataset.tab === id)); main.replaceChildren(views[id] || views.overview); };
  for (const [id, icon] of [["overview", "pi-chart-bar"], ["nodes", "pi-th-large"], ["guide", "pi-compass"]]) { const item = el("button", "cd-tab"); item.dataset.tab = id; item.append(ic(icon), document.createTextNode(t(id, language))); item.onclick = () => show(id); tabs.append(item); }
  root.append(tabs, main, el("footer", "cd-footer", `Continuity Director v${VER} · 20 nodes`)); host.append(root); show(host.dataset.cdTab || "overview");
}

function fallback() {
  if (document.querySelector(".cd-launcher")) return;
  const launcher = el("button", "cd-launcher", "CD"), panel = el("aside", "cd-floating");
  panel.hidden = true; launcher.onclick = () => { panel.hidden = !panel.hidden; if (!panel.hidden) render(panel); };
...
app.registerExtension({
  ...
  commands: [{ id: "continuity-director.add-starter", label: "Continuity Director: Add starter chain", icon: "pi pi-plus", function: starter }, { id: "continuity-director.focus", label: "Continuity Director: Focus nodes", icon: "pi pi-expand", function: focus }],
  ...
  async setup() { css(); try { if (app.extensionManager?.registerSidebarTab) app.extensionManager.registerSidebarTab({ id: "continuity-director", icon: "pi pi-video", title: "Continuity Director", tooltip: "Continuity Director", type: "custom", render }); else fallback(); } catch (error) { console.warn("[Continuity Director] sidebar fallback", error); fallback(); } },
  nodeCreated: style,
